    ...
    def get(self, sess):
        ...
        if self.cached_inception_real is None:
            ...
            self.cached_inception_real = {
                feat_key: np.concatenate([batch[feat_key] for batch in inception_real_batches], axis=0).astype(
                    np.float64)
...
class EvalWorker:
    def __init__(self, tpu_name, model_constructor, total_bs, dataset, inception_bs=8, num_inception_samples=1024,
        ...
        with self.strategy.scope():
            ...
            self.inception_real_train = InceptionFeatures(
                dataset=dataset.train_one_pass_input_fn(params={'batch_size': total_bs}), strategy=self.strategy,
                limit_dataset_size=limit_dataset_size // total_bs)
            # Val dataset, if it exists
            val_ds = dataset.eval_input_fn(params={'batch_size': total_bs})
            self.inception_real_val = None if val_ds is None else InceptionFeatures(dataset=val_ds,
            ...
            print('===== EMA =====')
            ema, _ = make_ema(global_step=self.global_step, ema_decay=1e-10,
                              trainable_variables=tf.trainable_variables())
    ...
    def run(self, logdir, once: bool, skip_non_ema_pass=True, dump_samples_only=False, load_ckpt=None, samples_dir=None,
            seed=0):
        ...
        with tf.Session(target=self.resolver.master(), config=config) as sess:
            ...
            for ckpt in ckpt_iterator:
                ...
                if dump_samples_only:
                    if not samples_dir:
                        samples_dir = os.path.join(eval_logdir,
                                                   '{}_samples{}'.format(type(self.dataset).__name__, global_step_val))
                    self._dump_samples(
                        sess, curr_step=global_step_val, samples_dir=samples_dir, ema=True)
                else:
                    self._write_eval_and_samples(sess, log=eval_log, curr_step=global_step_val, prefix='eval_ema',
                                                 ema=True)
                ...
                if not skip_non_ema_pass:
                    ...
                    if dump_samples_only:
                        self._dump_samples(
                            sess, curr_step=global_step_val, samples_dir=os.path.join(eval_logdir, 'samples'),
                            ema=False)
                    else:
                        self._write_eval_and_samples(sess, log=eval_log, curr_step=global_step_val, prefix='eval',
                                                     ema=False)
