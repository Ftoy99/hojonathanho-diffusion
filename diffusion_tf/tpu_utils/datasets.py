...
def load_cifar10_data():
    ...
    # Load data batches
    for i in range(1, 6):
        batch_path = os.path.join("cifar10", f"data_batch_{i}")
        with open(batch_path, 'rb') as fo:
            dict = pickle.load(fo, encoding='bytes')
            batches.append(dict)

    # Load meaningful label names
    batch_path = os.path.join("cifar10", f"batches.meta")
    with open(batch_path, 'rb') as fo:
        labels = pickle.load(fo, encoding='bytes')
        for index, label in enumerate(labels[b'label_names']):
            label_names[index] = label.decode() # this is bytes
